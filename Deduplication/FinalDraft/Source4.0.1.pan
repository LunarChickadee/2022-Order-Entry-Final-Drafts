...
///I think this shoudl fill anyway, and the question should be to automerge to the 
//////last record automatically? But let it go ahead and fill how it normally does?
...
Global Return_To,
    Seeds_Merged, Seeds_Hist1, Seeds_Hist2,
...
arrayfilter Seeds_Merged, Seeds_Merged, ",", val(array(Seeds_Hist1,seq(),","))+val(array(Seeds_Hist2, seq(), ","))
...
//____Got records with no Customer#? Give em one!
    lastrecord
    if val(«C#») < 1
        openfile "Customer#"
        call "newnumber"
...
yesno "Choose: "+arrayrange(array(list_of_records,info("matrixrow"),"~"), 2, 3,¶)+¶+"  to be the master record?"
...
global dedup_window, dedup_form_window

dedup_window=info("windownname")

openform "ChooseDestination"

dedup_form_window=info("windowname")
...
yesno "Ready to clear the selected records in the Deduplicator?"
